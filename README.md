...
Solution for [roadmap.sh SSH Remote Server Setup](https://roadmap.sh/projects/ssh-remote-server-setup) project.
...
```bash
ssh-keygen -t ed25519 -f ~/.ssh/roadmap_key1 -C "roadmap-key1"
...
| File | Purpose |
|---|---|
| `~/.ssh/roadmap_key1` | Private key #1 — **never share** |
| `~/.ssh/roadmap_key1.pub` | Public key #1 |
| `~/.ssh/roadmap_key2` | Private key #2 — **never share** |
| `~/.ssh/roadmap_key2.pub` | Public key #2 |
...
```bash
ssh-copy-id -i ~/.ssh/roadmap_key1.pub user@server-ip
...
```bash
ssh -i ~/.ssh/roadmap_key1 user@server-ip
